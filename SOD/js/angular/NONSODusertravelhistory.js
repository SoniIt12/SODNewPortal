var app = angular.module('NONsodUserTrHistory', ['ui.bootstrap', 'ngSanitize']);
...
app.controller("TravelHistory", function ($scope, $http, $timeout) {
    function loaddata() {
        $http({
            method: "GET",
            url: "../SodApprover/GetEmployeeBookingHistory_ByEmpCode?EmpId=0" + "&IsVendorBooking=true",
    ...
    $scope.downloadexcel = function () {
        ...
        if (result) {
            window.location = "../SodApprover/ExportVendorData?EmpId=0&IsVendorBooking=true&fdate=" + fdate + "&tdate=" + tdate;
        }
    };
    loaddata();
});

app.controller("VDTravelHistory", function ($scope, $http, $timeout) {
    $scope.filteritems = 0;
    $scope.getdatewiseList = function () {
        ...
        $http({
            method: "GET",
            url: "../SodApprover/GetEmployeeBookingHistory_ByEmpCode?EmpId=0&IsVendorBooking=true&fdate=" + fdate + "&tdate=" + tdate
        }).then(function mySucces(response) {
            $scope.trlist = response.data;
            $scope.currentPage = 1; //current page
            $scope.entryLimit = 10; //max no of items to display in a page
            $scope.filteredItems = $scope.trlist.length; //Initially for no filter  
            $scope.totalItems = $scope.trlist.length;
            $scope.filteritems = $scope.filteredItems;
        }, function myError(response) {
            $scope.myWelcome = response.statusText;
        });
    ...
    for (var i = 0; i < count; i++) {
        ...
        if ($("#checkboxhotel" + i).is(':checked')) {
            ...
            if ($("#txtCheckInDate" + i).val() == $("#txtCheckOutDate" + i).val()) {
                var checkIntime = ConvertTime($('#txtCheckintime' + i).val());
                var CheckoutTime = ConvertTime($('#txtCheckouttime' + i).val());
